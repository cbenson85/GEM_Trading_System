...
class PreCatalystFingerprint:
    ...
    def collect_all_fingerprints(self, input_file: str, output_file: str):
        ...
        print("ALL 10 DATA CATEGORIES - v4 (w/ Sector, Cash, Debt, Golden Cross)")
        print("="*60)
    ...
    def build_complete_fingerprint(self, explosion: Dict) -> Dict:
        ...
        print(f"  Collecting ALL 10 data categories for {ticker}...")
    ...
    def get_ticker_details_advanced(self, ticker: str, as_of_date: str) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                sector = self.map_sic_to_sector(sic_desc) # <-- NEW
    ...
    def get_stock_financials_advanced(self, ticker: str, as_of_date: datetime) -> Dict:
        ...
        --- UPDATED (v4) ---
        Now includes operating_expenses, assets, and liabilities.
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(results) >= 4:
                    ...
                    balance_q0 = q0.get('balance_sheet', {})
                    ...
                    operating_expenses_q0 = float(income_q0.get('operating_expenses', {}).get('value', 0) or 0)
                    ...
                    # --- NEW METRICS ---
                    total_assets = float(balance_q0.get('total_assets', {}).get('value', 0) or 0)
                    total_liabilities = float(balance_q0.get('total_liabilities', {}).get('value', 0) or 0)
                    ...
                    return {
                        ...
                        'operating_expenses': operating_expenses_q0,
                        'total_assets': total_assets, # <-- NEW
                        'total_liabilities': total_liabilities, # <-- NEW
                        'debt_to_asset_ratio': float(debt_to_asset_ratio) # <-- NEW
                    }
    ...
    def get_technical_analysis(self, ticker: str, start: str, end: str) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(bars) < 50: # Need 50 days for golden cross
                    return {'insufficient_data': True, 'note': 'Need 50+ bars for full TA'}
                ...
                is_golden_cross = price_ma20 > price_ma50 # <-- NEW
                ...
                return {
                    ...
                    'is_golden_cross': is_golden_cross, # <-- NEW
                    'volume_ma20': float(np.mean(volumes[-20:]))
                }
    ...
    def get_short_interest(self, ticker: str, catalyst_dt: datetime) -> Dict:
        ...
        try:
            ...
            older = short_reports[-1]  # Oldest report
            ...
            if response.status_code == 200:
                ...
                if results:
                    ...
                    unusual_strikes = [
                        ...
                        for r in results 
                        if r.get('day', {}).get('volume', 0) > avg_vol_per_strike * 5
                    ]
    ...
    def save_final_analysis(self, fingerprints: List, output_file: str):
        ...
        output = {
            ...
            'api_tier': 'Advanced - Complete (v4 w/ Sector, Cash, Debt, Golden Cross)',
            'data_categories': [
                'Company Profile (w/ Sector)',
                'Technical History (w/ Golden Cross)', 
                'Fundamentals (w/ Expenses & Debt)',
                'Relative Strength',
                'News Analysis',
                'Price Patterns',
                'Volume Profile',
        ...
        print("\n" + "="*60)
        print("COMPLETE ANALYSIS FINISHED (v4)")
        print("="*60)
        print(f"Total analyzed: {summary['total_analyzed']}")
        if 'percentages' in summary:
            print(f"  Short Interest Coverage: {summary['percentages']['short_data_coverage']:.1f}%")
            print(f"  Options Data Coverage: {summary['percentages']['options_data_coverage']:.1f}%")
            print(f"  Squeeze Setups Found: {summary['squeeze_setups']} ({summary['percentages']['squeeze_setup_pct']:.1f}%)")
...
def main():
    ...
    print(f"Starting COMPLETE Advanced Tier analysis (v4)...")
    print(f"Input: {input_file}")
    print(f"Collecting ALL 10 data categories (with new v4 metrics)...")
