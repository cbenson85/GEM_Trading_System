    ...
    inputs:
      mode:
        ...
        options:
          ...
          - small      # 50 stocks (10 batches × 5 stocks)  
          - medium     # 200 stocks (10 batches × 20 stocks)
          - full       # All 694 sustainable stocks (10 batches × ~70 stocks)
...
jobs:
  prepare-batches:
    ...
    - name: Split stocks into batches
      ...
      run: |
        ...
        # Use explosive_stocks_CLEAN.json for the 694 filtered stocks
        if [ "$MODE" = "test" ]; then
          echo "📊 TEST MODE: Splitting 10 stocks into 5 batches (2 stocks each)"
          ...
          echo "📊 SMALL MODE: Splitting 50 stocks into 10 batches (5 stocks each)"
          python phase3_batch_splitter.py Verified_Backtest_Data/explosive_stocks_CLEAN.json --batches 10 --limit 50
          echo "batch_count=10" >> $GITHUB_OUTPUT
        elif [ "$MODE" = "medium" ]; then
          echo "📊 MEDIUM MODE: Splitting 200 stocks into 10 batches (20 stocks each)"
          python phase3_batch_splitter.py Verified_Backtest_Data/explosive_stocks_CLEAN.json --batches 10 --limit 200
          echo "batch_count=10" >> $GITHUB_OUTPUT
        else
          echo "📊 FULL MODE: All 694 sustainable stocks into 10 batches"
  ...
  analyze-batches:
    ...
    - name: Analyze Batch ${{ matrix.batch }}
      ...
      run: |
        echo "🔬 Analyzing Batch ${{ matrix.batch }} with ALL data collectors..."
  ...
  merge-results:
    ...
    - name: Generate correlation matrix
      run: |
        echo "📊 Building correlation matrix with ALL data points..."
        python phase3_correlation_analyzer.py Verified_Backtest_Data/phase3_merged_analysis.json
    
    - name: Display results summary
      run: |
        echo "## 📊 Phase 3 Enhanced Analysis Complete" >> $GITHUB_STEP_SUMMARY
...
with open('Verified_Backtest_Data/phase3_correlation_matrix.json') as f:
    ...
    - name: Commit results
      run: |
        ...
        git diff --staged --quiet || git commit -m "🔬 Phase 3 Enhanced Analysis: ${{ github.event.inputs.mode }} mode
